...
def FR_frags_one_label(label_num, dict_tag, cam_list=[]):
    if dict_tag[1] == "F":
        part_labeled = cam_list[dict_tag[0][0]].frag_list[label_num]
        counterpart_labeled = cam_list[dict_tag[0][1]].frag_list[label_num]
        return part_labeled, counterpart_labeled

    elif dict_tag[1] == "R":
        part_labeled = cam_list[dict_tag[0][1]].frag_list[label_num]
        counterpart_labeled = cam_list[dict_tag[0][0]].frag_list[label_num]
        return part_labeled, counterpart_labeled

# 座標でdictを作る

def coordinate_dict_gen_one_label(label_num, tag, cam_list=[], dest_dir="temp"):
    """_summary_
    ...
    pair_coordinate = []
    part_labeled, counterpart_labeled = FR_frags_one_label(label_num, tag, cam_list)
    ...
    with open(pair_pt_file_path, "rb") as f:
        pair_pt_taged_labeled = pickle.load(f)

    for part_frag, pair, pt_idx in zip(part_labeled, pair_list_taged_labeled, pair_pt_taged_labeled):
        for each_pair, each_pt_idx in zip(pair, pt_idx):
            if each_pt_idx[0].size != 0:
                pair_coordinate.append(
                    (
                        np.array(
                            [
                                part_frag[each_pt_idx[0]],
                                counterpart_labeled[each_pair][each_pt_idx[1]],
                            ]
                        )
                    )
                )
...
def TDlines_gen_one_label(tag, cam_list=[], cam_pairs_F=[], src_dir="temp"):
    ...
    for pt in pts_labeled:
        ...
        temp_TDlines_labeled.append(tri_pt)
    ...
    with open(
        ...
        pickle.dump(temp_TDlines_labeled, f)
...
def reprojection_gen_one_label(tag, cam_list=[], tmp_dir="temp"):
    """
    Parameters
    ===================
    ...
    for P_tag in P_dict:
        ...
        with open(TDline_file_path, "rb") as f:
            TDlines_taged_labeled = pickle.load(f)

        for i, frag in enumerate(TDlines_taged_labeled):
...
def cal_distance_one_label(repro_sparse_P_labeled, contour_sparse_P_labeled, img_shape):
    dist_check_list_labeled = []
    ac_list_labeled = []
    for repro_frag in repro_sparse_P_labeled:
        repro_frag = repro_sparse_core(repro_frag, img_shape)
        supported, frag_ac = cal_distance_core(repro_frag, contour_sparse_P_labeled)
        dist_check_list_labeled.append(np.array(supported))
        ac_list_labeled.append(frag_ac)

    return ac_list_labeled, dist_check_list_labeled


def P_dict_check_one_label(label_num, repro_dict_taged_labeled, img_shape, cam_list=[]):
    ...
    P_list = []
    P_ac_list = []
    for P_tag in repro_dict_taged_labeled:
        repro_P = repro_dict_taged_labeled[P_tag]
        contour_P = cam_list[P_tag].contour_img[label_num]
        ac_list_labeled, dist_check_list_labeled = cal_distance_one_label(repro_P, contour_P, img_shape)
        P_list.append(dist_check_list_labeled)
        P_ac_list.append(ac_list_labeled)
    P_list = np.array(P_list)
    return P_ac_list, P_list


def P_check_integration_one_label(P_check):
    ...
    """
    temp = np.array(P_check > 0.8)  # 曲線中の何割が閾値以内か
    check_labeled = np.sum(np.array(temp), axis=0, dtype=np.uint16)
    return check_labeled


def ac_list_integration_one_label(P_ac_list):
    """交点毎のサポート
    ...
    """
    inter_ac = []
    for i in range(len(P_ac_list[0])):
        temp_array = np.zeros(len(P_ac_list[0][i]))
...
def gen_support_one_label(label_num, tag, cam_list=[], image_shape=(3456, 5184)):
    """_summary_
    ...
    P_ac_list, P_check = P_dict_check_one_label(label_num, repro_dict_taged, image_shape, cam_list)
    check_list = P_check_integration_one_label(P_check)
    inter_ac = ac_list_integration_one_label(P_ac_list)
